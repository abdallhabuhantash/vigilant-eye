  ...
  return (
    <div className="pointer-events-none absolute left-1/2 top-20 z-30 w-[min(92%,470px)] -translate-x-1/2 animate-alert-in border border-destructive/70 border-l-4 bg-background/92 shadow-[0_0_24px_color-mix(in_oklab,var(--destructive)_32%,transparent)] backdrop-blur-md">
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] gap-3 p-3">
        ...
        <div className="min-w-0"><p className="font-mono text-[9px] font-bold uppercase text-destructive">Critical alert · human review required</p><h3 className="mt-0.5 text-sm font-bold uppercase text-foreground">Suspicious Cheating Activity</h3><p className="text-xs text-muted-foreground">Mobile Phone Detected</p></div>
        <span className="border border-warning/40 px-1.5 py-0.5 font-mono text-[9px] text-warning">DEMO</span>
      </div>
      <dl className="grid grid-cols-3 border-t border-destructive/30 bg-destructive/5 font-mono text-[9px]">
        <div className="border-r border-destructive/20 p-2"><dt className="text-muted-foreground">PERSON / CONF.</dt><dd className="mt-0.5 text-foreground">ID 03 · {Math.round(event.confidence * 100)}%</dd></div>
        <div className="border-r border-destructive/20 p-2"><dt className="text-muted-foreground">DURATION</dt><dd className="mt-0.5 text-foreground">{event.durationSeconds.toFixed(1)} SEC</dd></div>
