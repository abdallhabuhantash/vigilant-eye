...
export function MainMonitoringViewport({ camera, detections, event, live, overlays, onToggleOverlays }: { camera: Camera; detections: DetectionOverlay[]; event?: DetectionEvent; live: boolean; overlays: boolean; onToggleOverlays: () => void }) {
  const frameRef = useRef<HTMLDivElement>(null);
  const capture = () => { const link = document.createElement("a"); link.href = examHallSurveillance; link.download = `${camera.name.replaceAll(" ", "-").toLowerCase()}-snapshot.jpg`; link.click(); };
  const fullscreen = () => { void frameRef.current?.requestFullscreen(); };
  return (
    <div ref={frameRef} className={cn("relative min-h-0 flex-1 overflow-hidden border border-primary/35 bg-background", event?.severity === "critical" && "animate-alert-frame border-destructive/70")}>
      <div className="absolute inset-0">
        {live ? <LiveStreamPlayer cameraId={camera.id} offline={camera.status === "offline"} /> : camera.status === "offline" ? <div className="hud-grid flex size-full flex-col items-center justify-center gap-2 text-destructive"><VideoOff className="size-8" /><span className="font-mono text-[10px] uppercase">Camera offline · No signal</span></div> : <><img src={examHallSurveillance} alt={`Demonstration surveillance view from ${camera.name}`} width={1536} height={864} className="size-full object-cover saturate-[0.72] contrast-[1.08] brightness-[0.72]" /><div className="absolute inset-0 bg-background/12" /></>}
      </div>
      <div className="pointer-events-none absolute inset-0 z-10 opacity-35" style={{ backgroundImage: "repeating-linear-gradient(to bottom, transparent 0, transparent 2px, color-mix(in oklab,var(--background) 28%,transparent) 3px)" }} />
      ...
      <div className="absolute bottom-5 right-5 z-40 flex items-center gap-3 border border-primary/40 bg-background/82 px-3 py-1.5 font-mono text-[9px] backdrop-blur-sm"><span className="flex items-center gap-1 text-primary"><Cpu className="size-3" /> AI</span><span>{camera.resolution}</span><span>{camera.fps} FPS</span><span className="text-warning">DEMO</span></div>
      {!live && camera.status !== "offline" && <div className="absolute bottom-16 left-1/2 z-30 -translate-x-1/2 bg-background/65 px-2 py-1 font-mono text-[8px] uppercase text-muted-foreground backdrop-blur-sm">Demo preview · Awaiting AI stream connection</div>}
    </div>
  );
}
...
export function CameraWall({ cameras, onSelect }: { cameras: Camera[]; onSelect: (camera: Camera) => void }) {
  return <div className="grid min-h-0 flex-1 grid-cols-2 gap-1 bg-background p-1">{cameras.slice(0, 4).map((camera) => <button key={camera.id} type="button" onClick={() => onSelect(camera)} className="group relative min-h-0 overflow-hidden border border-border bg-surface text-left"><img src={examHallSurveillance} alt={`Demo preview for ${camera.name}`} width={1536} height={864} className={cn("size-full object-cover brightness-50 transition group-hover:brightness-75", camera.status === "offline" && "grayscale")} /><div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-background/85 px-2 py-1 font-mono text-[9px]"><span>CH{String(camera.channel).padStart(2, "0")} · {camera.name}</span><span className={camera.status === "online" ? "text-success" : "text-destructive"}>{camera.status.toUpperCase()}</span></div><Grid2X2 className="absolute right-2 top-2 size-3.5 text-primary" /></button>)}</div>;
}