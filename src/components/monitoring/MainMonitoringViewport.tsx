  ...
  return (
    <div ref={frameRef} className={cn("relative min-h-0 flex-1 overflow-hidden border border-primary/35 bg-background", event?.severity === "critical" && "animate-alert-frame border-destructive/70")}>
      <div className="absolute inset-0">
        {camera.status === "offline" ? <div className="hud-grid flex size-full flex-col items-center justify-center gap-2 text-destructive"><VideoOff className="size-8" /><span className="font-mono text-[10px] uppercase">Camera offline · No signal</span></div> : <><img src={examHallSurveillance} alt={`Demonstration surveillance view from ${camera.name}`} width={1536} height={864} className="size-full object-cover saturate-[0.72] contrast-[1.08] brightness-[0.72]" /><div className="absolute inset-0 bg-background/12" />{live && <LiveStreamPlayer cameraId={camera.id} offline={false} />}</>}
      </div>
      ...
      {camera.status !== "offline" && <div className="absolute bottom-16 left-1/2 z-30 -translate-x-1/2 bg-background/65 px-2 py-1 font-mono text-[8px] uppercase text-muted-foreground backdrop-blur-sm">Demo preview · Awaiting AI stream connection</div>}
    </div>
  );
}
